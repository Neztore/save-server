...
const { hashRounds, adminUser } = require('../../config')
const fs = require('fs')
...
users.get('/:id/config', function (req, res) {
  // Generate config
  let isLink = false
  // TODO: Add isLink detection / "type" Query parameter.
  const config = {
    "Version": "12.4.1",
    "Name": `${req.target.username} ${isLink ? "Link shorten":"Upload"}-Save-Server`,
    "DestinationType": "ImageUploader, TextUploader, FileUploader",
    "RequestMethod": "POST",
    "RequestURL": `${req.secure ? "https":"http"}://${req.hostname}/files`, // will probably need to make this /api/files!
    "Headers": {
  ...
  res.set({"Content-Disposition":`attachment; filename="${req.target.username} ${isLink ? "Link shorten":"Upload"}"`});
})
